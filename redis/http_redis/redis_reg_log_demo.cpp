...
//--------------使用main测试-------------------//这里注释掉了，这一块儿是为了测试redis注册和登录
 int main(){
    ...
    bool reg=register_user(username,password,rconn);
    cout<<"注册"<<(reg?"成功":"失败")<<endl;
    bool log=login_user(username,password,rconn);
    cout<<"登录"<<(log?"成功":"失败")<<endl;
